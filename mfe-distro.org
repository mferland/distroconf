...
echo "deb [arch=amd64] https://weechat.org/ubuntu hirsute main" | sudo tee /etc/apt/sources.list.d/weechat.list
echo "deb-src [arch=amd64] https://weechat.org/ubuntu hirsute main" | sudo tee -a /etc/apt/sources.list.d/weechat.list
sudo apt-get update
...
sudo VBoxManage extpack install --replace Oracle_VM_VirtualBox_Extension_Pack-6.1.30.vbox-extpack
sudo usermod -aG vboxusers marc
rm Oracle_VM_VirtualBox_Extension_Pack-6.1.30.vbox-extpack
reboot
