...
sudo apt install -y git libtool gcc autotools-dev autoconf pkg-config zlib1g-dev check emacs25 i3 tree htop gitk texinfo texlive-full nfs-common gnuplot-x11 vlc-bin python-minimal dvdbackup mkisofs
mkdir -p ~/dev
cd ~/dev
...
sudo echo "deb http://download.virtualbox.org/virtualbox/debian artful contrib" >> /etc/apt/sources.list
